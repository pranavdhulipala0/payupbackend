...
app.post("/create", async (req, res) => {
  ...
  creation.save();
  for(var i = 0;i<usercount;i++){
    const temp = await authMod.updateOne({username: usersArray[i]},{ $push: { rooms: roomId }})
  }
...
app.get("/temproute",async (req,res)=>{
    const temp = await authMod.findOne({username:"drakeswd"})
    .then((data)=>{
        console.log(data);
        data=data.json;
    }
...
app.post("/addUser", async (req, res) => {
  ...
  Object.keys(paymentData.toBePaid).forEach(function (key) {
    ...
    const temp = await authMod.updateOne({username: new user},{ $push: { rooms: roomId }})
...
app.post("/split", async (req, res) => {
  ...
  Object.keys(paymentData.toBePaid[username]).forEach(function (key) {
    if (paymentData.toPay[key][username] == 0)
